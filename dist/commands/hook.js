...
export default class hook {
    constructor() {
        ...
        this.githooksdir = path.join(this.gitdir, "hooks");
        this.gitprepushhookdir = path.join(this.githooksdir, "pre-push");
    }
    MustHaveGitInitialized() {
        const gitdirExists = fs.existsSync(this.gitdir);
        if (!gitdirExists) {
            logreport.warn("git is not initialized in the current directory, cannot hook. run `git init` first.");
            return;
        }
    }
    unhookgit() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (!fs.existsSync(this.gitprepushhookdir)) {
                    logreport.warn("no `pre-push` hook was found.\n" +
                        path.relative(process.cwd(), this.gitprepushhookdir));
                    return;
                }
                const e = yield prompt({
                    name: "confirm_proceed",
                    type: "confirm",
                    message: "unhook git will remove any `pre-push` script whether or not it was created by lpm. Proceed?\n\n" +
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (existingPrePushHook) {
                    logreport.warn("an existing `pre-push` hook was already found.\n" +
                        path.relative(process.cwd(), this.gitprepushhookdir));
                    return;
                }
    ...
    build(program) {
        const hookprogram = program
            .command("hook")
            .description("Setup hooks for git and npm. Hooks help prepare the state of your package so you never publish with locally defined dependencies.")
            .action(() => {
            console.log("hook all!");
        });
        hookprogram
            .command("git")
            .description("setup git hooks so you never push to repository with locally defined dependencies.")
            .action(() => {
            this.hookgit();
        });
